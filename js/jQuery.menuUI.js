	...
	$.fn.menuUI = function(data, callBack, options) {
		...
		var menuUI = {
			...
			template: {
				...
				menuItem : '<li><a href="$action" id="$id">$value<span class="pull-right hidden-xs showopacity $icon"></span></a></li>',
		...
		for (var liData of jsonData.menu) {
			...
			(liData.submenu && liData.submenu.length >0) ? (menuItem = setSubMenu(liData)) :
				(menuItem = menuUI.template.menuItem.replace('$id', liData.id || '').replace('$value', liData.value || '').replace('$icon', liData.icon || '').replace('$action', liData.action || ''));
